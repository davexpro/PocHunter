...
def main(io_info, usage='', argvs=[]):
    ...
    try:
        ...
        if 'form-cred' in argvs:
            parser.add_option("--form-cred", dest="form_cred",
                              help="Form authentication credentials (user=guest&pwd=123456)")
        if 'payload' in argvs:
            parser.add_option("--payload", dest="payload", default="alert(1119)",
                              help="the payload ( alert(1119) / phpinfo() / and 1=2  )")
